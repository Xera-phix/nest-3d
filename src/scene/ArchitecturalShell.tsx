...
function DimensionMarks({ roomDimensions }: { roomDimensions: RoomDimensions }) {
  const widthOffset = roomDimensions.depth / 2 - 0.15
  const depthOffset = roomDimensions.width / 2 - 0.15

  return (
    <group position={[0, 0.045, 0]}>
      <Line
        points={[
          [-roomDimensions.width / 2, 0, widthOffset],
          [roomDimensions.width / 2, 0, widthOffset],
        ]}
      ...
      <Line
        points={[
          [depthOffset, 0, -roomDimensions.depth / 2],
          [depthOffset, 0, roomDimensions.depth / 2],
        ]}
        color={SCENE_COLORS.cobalt}
        lineWidth={1.2}
      ...
      <Text
        ...
        color="#244598"
      >
        {roomDimensions.depth.toFixed(2)} m
      </Text>
      {[-roomDimensions.width / 2, roomDimensions.width / 2].map((x) => (
        <Line
          key={`width-${x}`}
          points={[
      ...
      {[-roomDimensions.depth / 2, roomDimensions.depth / 2].map((z) => (
        <Line
          key={`depth-${z}`}
          points={[
...
function Window({ roomDimensions }: { roomDimensions: RoomDimensions }) {
  const windowX = Math.min(0.65, roomDimensions.width / 2 - 0.9)
  const windowY = Math.min(1.55, roomDimensions.height - 0.65)

  return (
    <group
      position={[windowX, windowY, -roomDimensions.depth / 2 + 0.01]}
    >
      <RoundedBox
...
function Door({ roomDimensions }: { roomDimensions: RoomDimensions }) {
  return (
    <group
      position={[-roomDimensions.width / 2 + 0.02, 1.03, -0.72]}
...
export function ArchitecturalShell() {
  ...
  return (
    <group>
      ...
      >
        <boxGeometry
          args={[roomDimensions.width + 0.16, 0.12, roomDimensions.depth + 0.16]}
        />
      </mesh>
      <Grid
      ...
      <group ref={wallGroup}>
        ...
        >
          <boxGeometry
            args={[roomDimensions.width + 0.12, roomDimensions.height, 0.12]}
          />
        </mesh>
        <mesh
          position={[
            -roomDimensions.width / 2 - 0.06,
            roomDimensions.height / 2,
            0,
          ]}
          castShadow
        ...
        >
          <boxGeometry
            args={[0.12, roomDimensions.height, roomDimensions.depth + 0.12]}
          />
        </mesh>
        <Window roomDimensions={roomDimensions} />
        <Door roomDimensions={roomDimensions} />
        <mesh
          position={[0, 0.065, -roomDimensions.depth / 2 + 0.02]}
          material={sceneMaterials.concrete}
        >
          <boxGeometry args={[roomDimensions.width, 0.13, 0.035]} />
        </mesh>
        <mesh
          position={[-roomDimensions.width / 2 + 0.02, 0.065, 0]}
          material={sceneMaterials.concrete}
        >
          <boxGeometry args={[0.035, 0.13, roomDimensions.depth]} />
        </mesh>
      </group>

      <group ref={dimensionGroup} scale={0.001} visible={false}>
        <DimensionMarks roomDimensions={roomDimensions} />
      </group>
      <Line
        points={[
          [
